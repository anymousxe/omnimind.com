const osCommandSets = {
  'Ubuntu Neural': {
    ...
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ifconfig','nano','clear','help','play','neuro-scan','dream-status','install-os','reboot','specs','browser','apps','download','websites','ai','cortex','agent-name','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  },
  'Arch Brain': {
    pkg: 'pacman', install: 'pacman -S', remove: 'pacman -R', update: 'pacman -Syu', upgrade: 'pacman -Syu',
    search: 'pacman -Ss', list: 'pacman -Q',
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ip','vi','clear','help','play','neuro-scan','dream-status','install-os','reboot','systemctl','journalctl','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  },
  'Mint Cerebral': {
    pkg: 'apt', install: 'apt install', remove: 'apt remove', update: 'apt update', upgrade: 'apt upgrade',
    search: 'apt search', list: 'apt list --installed',
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ifconfig','xed','clear','help','play','neuro-scan','dream-status','install-os','reboot','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  },
  'Fedora Synapse': {
    ...
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ip','vi','clear','help','play','neuro-scan','dream-status','install-os','reboot','selinux-status','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  },
  'Debian Thought': {
    pkg: 'apt', install: 'apt install', remove: 'apt remove', update: 'apt update', upgrade: 'apt upgrade',
    search: 'apt search', list: 'dpkg -l',
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ifconfig','nano','clear','help','play','neuro-scan','dream-status','install-os','reboot','dpkg','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  },
  'Gentoo Neural': {
    ...
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ip','vi','clear','help','play','neuro-scan','dream-status','install-os','reboot','equery','etc-update','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  },
  'Alpine Brain': {
    pkg: 'apk', install: 'apk add', remove: 'apk del', update: 'apk update', upgrade: 'apk upgrade',
    search: 'apk search', list: 'apk info',
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ifconfig','vi','clear','help','play','neuro-scan','dream-status','install-os','reboot','rc-service','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  },
  'Kali NeuroPen': {
    pkg: 'apt', install: 'apt install', remove: 'apt remove', update: 'apt update', upgrade: 'apt upgrade',
    search: 'apt search', list: 'apt list --installed',
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ifconfig','nano','clear','help','play','neuro-scan','dream-status','install-os','reboot','nmap','nikto','aircrack-ng','hydra','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  },
  'NixOS Thought': {
    ...
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ip','nano','clear','help','play','neuro-scan','dream-status','install-os','reboot','nixos-rebuild','nix-channel','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  },
  'Void Cortex': {
    ...
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ip','vi','clear','help','play','neuro-scan','dream-status','install-os','reboot','sv','xbps-query','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  },
  'NeuralBSD': {
    pkg: 'pkg', install: 'pkg install', remove: 'pkg delete', update: 'pkg update', upgrade: 'pkg upgrade',
    search: 'pkg search', list: 'pkg info',
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ifconfig','ee','clear','help','play','neuro-scan','dream-status','install-os','reboot','pkg','sysctl','freebsd-version','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  },
  'FreeCortex': {
    pkg: 'pkg', install: 'pkg install', remove: 'pkg delete', update: 'pkg update', upgrade: 'pkg upgrade',
    search: 'pkg search', list: 'pkg info',
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ifconfig','ee','clear','help','play','neuro-scan','dream-status','install-os','reboot','zpool','zfs','jls','pkg-create','pkg-upload','pkg-list','pkg-run','pkg-remove','create-os']
  }
};
...
function executeCommand(cmd, args, state) {
  ...
  switch (cmdLower) {
    case 'help': {
      ...
      out.push('');
      out.push('\x1b[1;36m Package Creation:\x1b[0m');
      out.push('  pkg-create <type> <name>  AI creates a package');
      out.push('    Types: game, malware, browser, engine, app, tool');
      out.push('  pkg-upload <name>  Upload package to neural repo');
      out.push('  pkg-list           List installed packages');
      out.push('  pkg-run <name>     Run an installed package');
      out.push('  pkg-remove <name>  Remove an installed package');
      out.push('  apt install <name> Install from repo (custom packages too)');
      out.push('');
      out.push('\x1b[1;36m Custom OS:\x1b[0m');
      out.push('  create-os <name>   AI generates a custom OS');
      out.push('  install-os <name>  Download an OS');
      out.push('  boot <name>        Boot a downloaded OS');
      out.push('  my-os              List downloaded OSes');
      out.push('');
    ...
    case 'install-os': {
      ...
      if (!target) {
        ...
        out.push('');
        out.push('\x1b[1;33mCustom OS:\x1b[0m');
        out.push('  create-os <name>   - AI generates a custom OS with unique commands');
        out.push('');
    ...
    case 'pkg-create': {
      const pkgType = args[0] || '';
      const pkgName = args.slice(1).join(' ');
      const validTypes = ['game', 'malware', 'browser', 'engine', 'app', 'tool'];
      if (!pkgType || !validTypes.includes(pkgType)) {
        out.push('\x1b[1;36m\xE2\x9C\xA8 Package Creator\x1b[0m');
        out.push('Usage: pkg-create <type> <name>');
        out.push('');
        out.push('\x1b[1;33mTypes:\x1b[0m');
        out.push('  game     \x1b[1;36m- AI generates a playable game\x1b[0m');
        out.push('  malware  \x1b[1;31m- AI generates fictional malware (in-game)\x1b[0m');
        out.push('  browser  \x1b[1;34m- AI generates a custom browser\x1b[0m');
        out.push('  engine   \x1b[1;35m- AI generates a custom search engine\x1b[0m');
        out.push('  app      \x1b[1;32m- AI generates a terminal app\x1b[0m');
        out.push('  tool     \x1b[1;36m- AI generates a system tool\x1b[0m');
        out.push('');
        out.push('Example: pkg-create game neural-quest');
      } else if (!pkgName) {
        out.push('\x1b[1;33mProvide a name: pkg-create ' + pkgType + ' <name>\x1b[0m');
      } else {
        return { createPackage: { type: pkgType, name: pkgName } };
      }
      break;
    }
    case 'pkg-upload': {
      const pkgName = args.join(' ');
      if (!pkgName) { out.push('\x1b[1;33mUsage: pkg-upload <package-name>\x1b[0m'); out.push('Uploads a package you created to the neural repository.'); break; }
      return { uploadPackage: pkgName };
    }
    case 'pkg-list': {
    ...
    case 'pkg-run': {
      const pkgName = args.join(' ');
      if (!pkgName) { out.push('\x1b[1;33mUsage: pkg-run <package-name>\x1b[0m'); break; }
      return { runPackage: pkgName };
    }
    case 'pkg-remove': {
      const pkgName = args.join(' ');
      if (!pkgName) { out.push('\x1b[1;33mUsage: pkg-remove <package-name>\x1b[0m'); break; }
      return { removePackage: pkgName };
    }
    case 'create-os': {
      const osName = args.join(' ');
      if (!osName) {
        out.push('\x1b[1;35m\xE2\x9C\xA8 Custom OS Creator\x1b[0m');
        out.push('Usage: create-os <name>');
        out.push('');
        out.push('AI will generate a custom OS with:');
        out.push('  \x1b[1;32m\xE2\x9C\x93\x1b[0m Unique package manager');
        out.push('  \x1b[1;32m\xE2\x9C\x93\x1b[0m Custom exclusive commands');
        out.push('  \x1b[1;32m\xE2\x9C\x93\x1b[0m Boot screen & ASCII art');
        out.push('  \x1b[1;32m\xE2\x9C\x93\x1b[0m Custom kernel & version');
        out.push('');
        out.push('Example: create-os DreamOS-Quantum');
      } else {
        return { createOS: osName };
      }
      break;
    }
    default: {
      ...
      if (cmdLower === pkgCmd || cmd.startsWith(shell.install.split(' ')[0]) || cmd.startsWith(shell.remove.split(' ')[0]) || cmd.startsWith(shell.update.split(' ')[0]) || cmd.startsWith(shell.upgrade.split(' ')[0])) {
        ...
        if (subcmd === 'create') {
          const pkgType = args[1] || '';
          const pkgName = args.slice(2).join(' ');
          const validTypes = ['game', 'malware', 'browser', 'engine', 'app', 'tool'];
          if (!pkgType || !validTypes.includes(pkgType) || !pkgName) {
            out.push('\x1b[1;33mUsage: ' + cmd + ' create <type> <name>\x1b[0m');
            out.push('Types: game, malware, browser, engine, app, tool');
          } else {
            return { createPackage: { type: pkgType, name: pkgName } };
          }
          ...
          const pkgName = args.slice(1).join(' ');
          if (!pkgName) { out.push('\x1b[1;33mUsage: ' + cmd + ' upload <name>\x1b[0m'); }
          else return { uploadPackage: pkgName };
        } else if (subcmd === 'list' || subcmd === 'info') {
          return { listPackages: true };
        } else if (subcmd === 'run') {
          const pkgName = args.slice(1).join(' ');
          if (!pkgName) { out.push('\x1b[1;33mUsage: ' + cmd + ' run <name>\x1b[0m'); }
          else return { runPackage: pkgName };
        ...
        } else if (subcmd === 'install' || subcmd === 'add' || !subcmd && cmdLower === 'emerge' && args[0]) {
          const pkg = args.filter(a => !a.startsWith('-') && a !== 'install' && a !== 'add').join(' ') || 'unknown-pkg';
        ...
        } else if (cmdLower.includes('update') || cmdLower.includes('sync') || cmdLower.includes('check-update') || cmdLower.includes('channel')) {
