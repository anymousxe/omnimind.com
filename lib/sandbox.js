const osCommandSets = {
  'Ubuntu Neural': {
    ...
    commands: ['ls','cd','cat','mkdir','rm','cp','mv','pwd','whoami','uname','neofetch','top','ps','kill','ping','ifconfig','nano','clear','help','play','neuro-scan','dream-status','install-os','reboot','specs','browser','apps','download','websites','ai']
  },
  'Arch Brain': {
...
function executeCommand(cmd, args, state) {
  ...
  switch (cmdLower) {
    case 'help': {
      ...
      out.push('  install-os         Install a new OS');
    ...
    case 'ai': {
      ...
      if (!prompt) { out.push('\x1b[1;36m🧠 Cortex-Assistant (in-terminal)\x1b[0m'); out.push('Usage: ai <your question>'); out.push('Ask the AI to build parts, check compatibility, create games, etc.'); break; }
      return { ai: prompt };
    }
    default: {
